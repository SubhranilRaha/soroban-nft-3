...
// #[inline(always)]
// #[no_mangle]
// #[cfg_attr(target_arch = "wasm32", optimize(speed))]
// #[cfg(feature = "std")]
...
.map_or(1, |count: u32| count.checked_add(1)
...
fn _set_collection_info(
    ...
    if collection_info.name.as_ref().map_or(false, |name| name.len() > MAX_NAME_LENGTH) ||
   collection_info.description.as_ref().map_or(false, |desc| desc.len() > MAX_DESCRIPTION_LENGTH) ||
   collection_info.external_link.as_ref().map_or(false, |link| link.len() > MAX_LINK_LENGTH) ||
   collection_info.royalty_info.as_ref().map_or(false, |royalty| royalty.share > MAX_ROYALTY_SHARE)
{
    panic_with_error!(env, Error::InvalidCollectionInfo);
}
